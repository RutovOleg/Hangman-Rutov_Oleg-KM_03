# Problem Set 2, hangman_complex.py
# Name: Рутов Олег, КМ-03
# Collaborators: Плиско Єлизавета, КМ-03
# Time spent: приблизно 5,5 години
...
def i_match_with_gaps(my_word, other_word, letters_guessed):
    '''
    Improved version of function match_with_gaps:
        It's does not consider a match to words containing
    ...
    if len(my_word) == len(other_word):
        for checking_index in range(0, len(my_word)):
            if my_word[checking_index] != '_':
                if my_word[checking_index] != other_word[checking_index]:
                    return False
            elif other_word[checking_index] in letters_guessed:
                return False
        return True
    else:
        return False
...
def i_show_possible_matches(my_word, letters_guessed):
    '''
    Improved version of function show_possible_matches:
        Works with improved version of match_with_gaps, that
        does not consider a match to words containing incorrectly guessed letters,
        instead of base match_with_gaps function.
    ...
    print('Possible word matches are: ', end='')
    for checking_word in wordlist:
        if i_match_with_gaps(my_word, checking_word, letters_guessed):
            matches_not_found = False
            print(checking_word, end=' ')
    if matches_not_found:
        print('no matches found in basic words library')
...
def hangman(secret_word, hints_mode=0):
    '''
    secret_word: string, the secret word to guess.
    hints_mode: number of active hints mode:
        0 - hints disabled
        1 - enabled basic hints function
        2 - enabled improved hints function
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    while guesses_remaining > 0:
        ...
        if guessed_letter == '*' and hints_mode != 0:
            if hints_mode == 1:
                show_possible_matches(get_guessed_word(secret_word, letters_guessed))
            elif hints_mode == 2:
                i_show_possible_matches(get_guessed_word(secret_word, letters_guessed), letters_guessed)
        else:
...
def hangman_with_hints(secret_word):
    ...
    * If the guess is the symbol *, print out all words in wordlist that
    ...
    Follows the other limitations detailed in the problem write-up.
    '''
    hangman(secret_word, 1)



def hangman_with_improved_hints(secret_word):
    '''
    secret_word: string, the secret word to guess.
...
if __name__ == "__main__":
    ...
    ## Start hangman game with hints
    # hangman_with_improved_hints(secret_word)
    ...
    while True:
        ...
        print('* To play hangman with random word without hints, enter "rd"'
              ...
              '\n* To play hangman with random word and improved hints, enter "ri"'
              '\n* To play hangman with your word without hints, enter "ed"'
              '\n* To play hangman with your word and hints, enter "eh"'
              '\n* To play hangman with your word and improved hints, enter "ei"'
